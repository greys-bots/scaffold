import {
	confirmVals as STRINGS,
	confirmReacts as REACTS,
	confirmBtns as BUTTONS,
	numbers as NUMBERS
} from '../extras.js';

export async function genEmbeds(bot, arr, genFunc, info = {}, fieldnum, extras = {}) {
...
export function genComps(arr, fn, limit = 10) {
	let comps = [];
	let cur = [];
...
export async function paginateEmbeds(bot, m, reaction) {
...
export async function getConfirmation(bot, msg, user) {
...
export async function getChoice(bot, msg, user, choices = [
	{
		name: 'yes',
		accepted: ['confirm', '✅']
	},
...
export async function awaitMessage(bot, msg, user, time) {
	return new Promise(res => {
		function msgListener(message) {
			if (message.channel.id != msg.channel.id ||
				message.author.id != user.id) return;
...
export async function awaitModal(ctx, data, user, ephemeral = false, time = 30000) {
	return new Promise(async (res) => {
